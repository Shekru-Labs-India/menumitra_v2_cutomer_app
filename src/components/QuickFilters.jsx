    ...
    if (isOpen) {
      ...
      Object.keys(dropdownRefs).forEach(key => {
        if (key !== dropdownName && dropdownRefs[key].current) {
          const dropdown = dropdownRefs[key].current.querySelector('.dropdown-menu');
        ...
        <div className="card-body p-0">
          <div className="basic-dropdown">
            <div className="dropdown">
              <button 
                type="button" 
                className={`btn ${activeValue && activeValue !== 'all' ? 'btn-success' : 'btn-outline-success'} dropdown-toggle`}
              ...
              <div 
                className={`dropdown-menu ${openDropdown === dropdownType ? 'show' : ''}`}
              >
                {options.map((option) => (
                  <a
                    key={option.id}
                    className={`dropdown-item ${activeValue === option.id ? 'active' : ''}`}
